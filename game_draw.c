...
void pic8x16_Draw(uint8_t frame,uint8_t *pic)
{
	...
	for (i=0;i<16;i++)
	{
		...
		if ((b&(1<<0))>0) g_SetPixel(0,y);
		if ((b&(1<<1))>0) g_SetPixel(1,y);
		if ((b&(1<<2))>0) g_SetPixel(2,y);
		if ((b&(1<<3))>0) g_SetPixel(3,y);
		if ((b&(1<<4))>0) g_SetPixel(4,y);
		if ((b&(1<<5))>0) g_SetPixel(5,y);
		if ((b&(1<<6))>0) g_SetPixel(6,y);
		if ((b&(1<<7))>0) g_SetPixel(7,y);
		y++;
	};
	g_Update();
}
...
void RandomGameOver(void)
{
	...
	if (n==0)
	{
		ClearAnimation002(); return;
	};
	if (n==1)
	{
		ClearAnimation003(); return;
	};
	if (n==2)
	{
		ClearAnimation000(); return;
	};
	if (n==3)
	{
		ClearAnimation001(); return;
	};
};
...
void GamePause(void)
{
	...
	while (1)
	{
		...
		{
			v_DelayCount=millis(); sei();v_rnd++;
			if (sw==0)
			{
				sw=1; pic8x16_Draw(0,animation_008);
			}
			else
			{
				sw=0; g_Clear(); g_Update();
			};
		};
		//sei();
		};
	...
	while (1)
	{
		...
		if (pReadKeys()>0)
		{
			...
			{//Up
				...
				GameSetAnimSize(&v_GameNumber,&v_AnimSize);
				v_AnimFrame=0; v_AnimCount=millis();
				v_Redraw=1; v_rnd++;
			};
			if (pKeySwitch(0x02)==2)
			{//Left
				if (v_LevelSpeed>0) v_LevelSpeed--;
			...
			{//Right
				if (v_LevelSpeed<4) v_LevelSpeed++;
				v_CicleTime=GameSpeedCount();
				v_Redraw=1; v_rnd++;
			};
			if (pKeySwitch(0x10)==2)
		...
		{
			v_AnimCount=millis(); v_Redraw=1; v_rnd++;
		...
		if (v_Redraw>0)
		{
			switch (v_GameNumber)
			{
				case 0:{pic8x16_Draw(v_AnimFrame,animation_000); break;};
				case 1:{pic8x16_Draw(v_AnimFrame,animation_001); break;};
				case 2:{pic8x16_Draw(v_AnimFrame,animation_002); break;};
				case 3:{pic8x16_Draw(v_AnimFrame,animation_003); break;};
				case 4:{pic8x16_Draw(v_AnimFrame,animation_004); break;};
				case 5:{pic8x16_Draw(v_AnimFrame,animation_005); break;};
				case 6:{pic8x16_Draw(v_AnimFrame,animation_006); break;};
			};
			v_Redraw=0;
		};
		...
		if (v_RunGame>0)
		{
			switch (v_GameNumber)
			{
				case 0:{bricks_Game(0); break;};
				case 1:{bricks_Game(1); break;};
				case 2:{bricks_Game(2); break;};
				case 3:{snake_Game(); break;};
				case 4:{shoot_Game(); break;};
				case 5:{drive_Game(); break;};
				case 6:{maze_Game(); break;};
			};
			v_RunGame=0;
		};
	};
